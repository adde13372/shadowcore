      ...
      _async_threads(0), _synch_threads(0)
{
    ...
    WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION, "ShadowCore does not support MySQL versions below 5.1");
    WPFatal(mysql_get_client_version() == MYSQL_VERSION_ID, "Used MySQL library version (%s) does not match the version used to compile ShadowCore (%s). Search on forum for TCE00011.",
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
            TC_LOG_ERROR("sql.driver", "ShadowCore does not support MySQL versions below 5.1");
            return 1;
        }
        else
        {
