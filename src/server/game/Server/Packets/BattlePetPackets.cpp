...
{
    data << (petBattleSlot.Pet.Guid.IsEmpty() ? ObjectGuid::Create<HighGuid::BattlePet>(0) : petBattleSlot.Pet.Guid);
...
{
    data << battlePet.Guid;
    data << battlePet.Species;
    data << battlePet.CreatureID;
    data << battlePet.CollarID;
    data << battlePet.Breed;
    data << battlePet.Level;
    data << battlePet.Exp;
    data << battlePet.Flags;
    data << battlePet.Power;
    data << battlePet.Health;
    data << battlePet.MaxHealth;
    data << battlePet.Speed;
    data << battlePet.Quality;
    data.WriteBits(battlePet.Name.size(), 7);
    data.WriteBit(battlePet.OwnerInfo.is_initialized());
    data.WriteBit(battlePet.Name.empty());
    data.FlushBits();

    data.WriteString(battlePet.Name);

    if (battlePet.OwnerInfo)
    {
        data << battlePet.OwnerInfo->Guid;
        data << uint32(battlePet.OwnerInfo->PlayerVirtualRealm);
        data << uint32(battlePet.OwnerInfo->PlayerNativeRealm);
    }
