...
// such checks will speedup ShadowCore change amount/send for client operations
...
NonDefaultConstructible<pAuraEffectHandler> AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleModFixate,                                 //128 SPELL_AURA_MOD_FIXATE
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //484 SPELL_AURA_ALLOW_INTERRUPT
    ...
    &AuraEffect::HandleNULL,                                      //494
    &AuraEffect::HandleNULL,                                      //495
    &AuraEffect::HandleNULL,                                      //496
    &AuraEffect::HandleNULL,                                      //497
    &AuraEffect::HandleNULL,                                      //498
    &AuraEffect::HandleNULL,                                      //499
    &AuraEffect::HandleNULL,                                      //500
    &AuraEffect::HandleNULL,                                      //501
    &AuraEffect::HandleNULL,                                      //502
    &AuraEffect::HandleNULL,                                      //503
};
...
void AuraEffect::HandleAuraLinked(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (mode & AURA_EFFECT_HANDLE_REAL)
    {
        if (apply)
        {
            ...
            if (GetAmount())
                caster->CastCustomSpell(target, triggeredSpellId, &m_amount, nullptr, true, nullptr, this);
            else
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (GetSpellInfo()->Id)
            {
                ...
                case 22842:
                {
                    ...
                    target->CastCustomSpell(target, 22845, &regen, nullptr, true, nullptr, this);
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    else
    {
        ...
        switch (auraId)
        {
            ...
            // Mana Tide
            case 16191:
                target->CastCustomSpell(target, triggerSpellId, &m_amount, nullptr, true, nullptr, this);
                return;
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
        ...
        && m_spellInfo->SpellFamilyFlags[0] & 0x00000010)
    {
        ...
        if (manaFeedVal > 0)
        {
            ...
            caster->CastCustomSpell(caster, 32554, &feedAmount, nullptr, true, nullptr, this);
        }
    }
...
void AuraEffect::HandleProcTriggerSpellWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId, GetBase()->GetCastDifficulty()))
    {
        ...
        triggerCaster->CastCustomSpell(triggerTarget, triggerSpellId, &basepoints0, nullptr, true, nullptr, this);
    }
    else
...
void AuraEffect::HandleModFixate(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    if (apply)
        caster->TauntApply(target, true);
    else
        caster->TauntFadeOut(caster);
}
