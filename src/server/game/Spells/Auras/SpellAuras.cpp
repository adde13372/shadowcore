...
bool Aura::ModStackAmount(int32 num, AuraRemoveMode removeMode /*= AURA_REMOVE_BY_DEFAULT*/, bool resetPeriodicTimer /*= true*/, bool refresh /*= true*/)
{
    ...
    refresh = stackAmount >= GetStackAmount() && (m_spellInfo->StackAmount || !m_spellInfo->HasAttribute(SPELL_ATTR1_DONT_REFRESH_DURATION_ON_RECAST));
            ...
            case SPELLFAMILY_DRUID:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x10 && GetEffect(EFFECT_0))
                {
                    ...
                    if (caster->HasAura(64760))
                    {
                        ...
                        caster->CastCustomSpell(target, 64801, &heal, nullptr, true, nullptr, GetEffect(EFFECT_0));
                    }
                }
                break;
        }
                ...
                if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL && GetSpellInfo()->SpellFamilyFlags[0] & 0x00000001)
                {
                    ...
                    if (Aura const* aura = caster->GetAuraOfRankedSpell(47535))
                    {
                        ...
                        if (AuraEffect const* aurEff = aura->GetEffect(0))
                        {
                            ...
                            caster->CastCustomSpell(caster, 47755, &basepoints0, nullptr, true);
                        }
                    }
                }
                break;
