...
void Spell::SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32& processedEffectMask)
{
    ...
    switch (targetType.GetSelectionCategory())
    {
                ...
                case TARGET_OBJECT_TYPE_SRC:
                    switch (targetType.GetReferenceType())
                    {
                        case TARGET_REFERENCE_TYPE_CASTER:
                            ...
                            break;
                        default:
                            TC_LOG_ERROR("Spell", "Spell::SelectEffectImplicitTargets: received not implemented select target reference type for TARGET_TYPE_OBJECT_SRC / Spell ID = %u and Effect = %u and target type = %u \n", m_spellInfo->Id, effIndex, targetType.GetTarget());
                            break;
                    }
                    break;
                case TARGET_OBJECT_TYPE_DEST:
                     switch (targetType.GetReferenceType())
                     {
                         ...
                         case TARGET_REFERENCE_TYPE_DEST:
                             ...
                             break;
                         default:
                             TC_LOG_ERROR("Spell", "Spell::SelectEffectImplicitTargets: received not implemented select target reference type for TARGET_TYPE_OBJECT_DEST / Spell ID = %u and Effect = %u and target type = %u \n", m_spellInfo->Id, effIndex, targetType.GetTarget());
                             break;
                     }
                     break;
                default:
                    switch (targetType.GetReferenceType())
                    {
                        ...
                        case TARGET_REFERENCE_TYPE_TARGET:
                            ...
                            break;
                        default:
                            TC_LOG_ERROR("Spell", "Spell::SelectEffectImplicitTargets: received not implemented select target reference type for TARGET_TYPE_OBJECT / Spell ID = %u and Effect = %u and target type = %u \n", m_spellInfo->Id, effIndex, targetType.GetTarget());
                            break;
                    }
                    break;
            }
        ...
        case TARGET_SELECT_CATEGORY_NYI:
            ...
            break;
        default:
            TC_LOG_ERROR("Spell", "Spell::SelectEffectImplicitTargets: received not implemented select target category / Spell ID = %u and Effect = %u and target type = %u \n", m_spellInfo->Id, effIndex, targetType.GetTarget());
            break;
    }
}
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        Trinity::WorldObjectSpellConeTargetCheck check(DegToRad(m_spellInfo->ConeAngle), m_spellInfo->Width ? m_spellInfo->Width : m_caster->GetObjectSize(), radius, m_caster, m_spellInfo, selectionType, condList);
        ...
        case TARGET_DEST_DB:
            if (SpellTargetPosition const* st = sSpellMgr->GetSpellTargetPosition(m_spellInfo->Id, effIndex))
            {
                ...
                if (m_spellInfo->HasEffect(SPELL_EFFECT_TELEPORT_UNITS) ||
                    m_spellInfo->HasEffect(SPELL_EFFECT_RITUAL_ACTIVATE_PORTAL) ||
...
bool WorldObjectSpellConeTargetCheck::operator()(WorldObject* target)
{
    ...
    {
        if (!_caster->HasInLine(target, target->GetObjectSize(), _lineWidth))
            return false;
    }
    else
    {
