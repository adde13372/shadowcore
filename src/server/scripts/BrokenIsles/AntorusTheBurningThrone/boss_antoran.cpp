...
class spell_command_felshield_or_beacon : public SpellScript
{
    ...
    void HandleScript(SpellEffIndex /*effectIndex*/)
    {
        ...
        if (GetSpellInfo()->Id == SPELL_FELSHIELD_EMITTER)
            GetCaster()->CastSpell(GetCaster()->GetPosition(), SPELL_FELSHIELD_EMITTER_SUM, true, nullptr, nullptr, owner->GetGUID());
        else if (GetSpellInfo()->Id == SPELL_DISRUPTOR_BEACON)
            GetCaster()->CastSpell(GetCaster()->GetPosition(), SPELL_DISRUPTOR_BEACON_SUM, true, nullptr, nullptr, owner->GetGUID());
    }
