    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        if (spell->Id == SPELL_DARK_FISSURE)
        {
            ...
            threatList.remove_if([this](HostileReference* ref) -> bool
            {
                ...
                if (auto unit = ObjectAccessor::GetUnit(*me, ref->getUnitGuid()))
                    if (!unit->IsPlayer())
                        return true;
            ...
            if (playerCount >= 5)
                Trinity::Containers::RandomResize(threatList, playerCount / 5);
            else
                Trinity::Containers::RandomResize(threatList, 1);
            ...
            for (auto ref : threatList)
                ...
                {
                    me->CastSpell(player, SPELL_DARK_FISSURE_AT, true);
                    delayTimer += 100;
                }
        }
