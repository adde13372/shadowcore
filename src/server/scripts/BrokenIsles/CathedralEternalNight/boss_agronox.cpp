                ...
                case EVENT_FLUMINATING_2:
                {
                    ...
                    for (Map::PlayerList::const_iterator itr = pPlayers.begin(); itr != pPlayers.end(); ++itr)
                        if (Player* player = itr->GetSource())
                            me->SummonCreature(NPC_FLUMINATING_LASHERS, adds_pos[cur_position++], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000);
                    break;
                }
                ...
                case EVENT_CHOKING_VINES:
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
                        if (Player* player = target->ToPlayer())
                            me->SummonCreature(NPC_CHOKING_VINES, vines_pos[urand(0, 3)], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000);
