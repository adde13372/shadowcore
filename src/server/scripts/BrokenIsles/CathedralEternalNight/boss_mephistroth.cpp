    ...
    struct boss_mephistrothAI : public BossAI
    {
        ...
        void DefaultEvents(uint8 phase)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                events.RescheduleEvent(EVENT_PHASE_TWO_TICK, me->GetMap()->GetDifficultyID() == DIFFICULTY_HEROIC ? 5000 : 4000);
                break;
            }
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            if (summon->GetEntry() == NPC_SHADOW_OF_MEPHISTROTH)
                if (illidan_saved)
                {
                    ...
                    summon->SetFacingToObject(illidan_saved);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (events.ExecuteEvent() == EVENT_1)
            {
                if (me->GetPower(POWER_ALTERNATE_POWER) < 300)
                {
                    events.RescheduleEvent(EVENT_1, 1000);
                    if (urand(1, 4) == 1)
                        me->SetFacingTo(frand(0, 2 * 3.14f));
                }
                else
                {
    ...
    class spell_mephistroth_carrion_swarm_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if (caster && target)
            {
                ...
                for (uint8 i = 0; i < 3; i++)
                {
                    caster->GetNearPoint2D(x,y, hit_range, angle);
