...
struct boss_cenarius : public BossAI
{
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        me->SummonCreature(NPC_MALFURION_STORMRAGE, me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN);
...
struct npc_cenarius_forces_of_nightmare : public ScriptedAI
{
    ...
    void SpellHitTarget(Unit* target, const SpellInfo* spell) override
    {
        ...
        case SPELL_ROTTEN_BREATH_FILTER:
            ...
            if (auto rotten = me->FindNearestCreature(NPC_ROTTEN_BREATH_TRIG, 100.0f))
                me->CastSpell(rotten, SPELL_ROTTEN_BREATH_DMG, false);
            break;
        }
    }
    ...
    void SwitchTarget(bool allowTank = false)
    {
        ...
        auto pTarget = SelectTarget(SELECT_TARGET_RANDOM, 1, [this](Unit const* t) { return t->IsPlayer(); });
        if (!pTarget)
    ...
    void JustDied(Unit* /*killer*/) override
    {
        if (me->GetOwner())
            if (Creature* owner = me->GetOwner()->ToCreature())
                owner->AI()->EnterEvadeMode(EVADE_REASON_NO_HOSTILES);
    }
        ...
        else if (!caster->HasUnitState(UNIT_STATE_CASTING))
        {
            ...
            case NPC_NIGHTMARE_ANCIENT:
                if (caster->GetMap()->IsMythic())
                {
                    ...
                    caster->CastSpell(caster, SPELL_DESICCATING_STOMP_MYTHIC, false);
                }
                else
...
class spell_cenarius_ancient_dream : public AuraScript
{
    ...
    void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        if (auto target = GetTarget())
        {
            ...
            target->SetHealth(target->CountPctFromMaxHealth(GetSpellInfo()->GetEffect(EFFECT_1)->BasePoints));
    ...
    void Register() override
    {
        ...
        OnEffectAbsorb += AuraEffectAbsorbFn(spell_cenarius_ancient_dream::Absorb, EFFECT_0);
    }
};
...
class spell_cenarius_twisted_touch_of_life_filter : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targetsList)
    {
        ...
        tempCreature.sort(Trinity::HealthPctOrderPred(true));
    ...
    {
        if (GetCaster() && GetHitUnit())
            GetCaster()->CastSpell(GetHitUnit(), GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints);
    }
...
class spell_cenarius_dread_thorns_reflects : public AuraScript
{
    ...
    void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        float damage = CalculatePct(eventInfo.GetDamageInfo()->GetDamage(), GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints);
        if (!damage)
            return;
    ...
    void OnUpdate(uint32 diff) override
    {
        if (createScaleTimer)
        {
            if (createScaleTimer <= diff)
            {
                ...
                if (at->GetObjectScale() < createScale)
                    at->SetObjectScale(0.15f);
                else
                    createScaleTimer = 0;
            }
            else
