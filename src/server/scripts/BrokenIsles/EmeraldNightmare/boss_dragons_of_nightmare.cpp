...
struct boss_dragon_ysondre : public BossAI
{
    boss_dragon_ysondre(Creature* creature) : BossAI(creature, DATA_DRAGON_NIGHTMARE)
    {
        ...
        Trinity::Containers::RandomResize(helpersList, IsMythicRaid() ? 3 : 2);
    }
    ...
    void Reset() override
    {
        ...
        me->SummonCreature(helpersList[0], dragonPos[0]);
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_1:
            if (auto owner = me->GetOwner())
                owner->GetAI()->DoAction(ACTION_2); //Leave group
            events.Reset();
            me->AttackStop();
            me->SetCanFly(true);
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        else if (id == 2)
        {
            ...
            if (auto owner = me->GetOwner())
            {
                ...
                owner->GetAI()->DoAction(ACTION_1); //Invite group
            }
        }
    }
        ...
        case ACTION_1:
            if (auto owner = me->GetOwner())
                if (auto summoner = owner->ToCreature())
                    summoner->AI()->DoAction(ACTION_2); //Leave group
            events.Reset();
            me->AttackStop();
            me->SetCanFly(true);
    ...
    void UpdateAI(uint32 diff) override
    {
        if (checkBossStateTimer)
        {
            if (checkBossStateTimer <= diff)
            {
                ...
                if (auto lethon = instance->instance->GetCreature(instance->GetGuidData(NPC_LETHON)))
                {
                    if (lethon->HasUnitFlag(UNIT_FLAG_NOT_ATTACKABLE_1))
                    {
                        me->DespawnOrUnsummon();
                        return;
                    }
