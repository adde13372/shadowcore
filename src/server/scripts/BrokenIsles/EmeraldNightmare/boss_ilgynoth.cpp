...
struct boss_ilgynoth : public BossAI
{
    ...
    void RemoveAT()
    {
        ...
        me->GetAreaTriggerListWithSpellIDInRange(list, 11189, 150.0f); //Nightmare Corruption - 208931
        for (auto at : list)
            at->Remove();
    }
    ...
    {
        _EnterCombat();
        me->SummonCreature(NPC_EYE_OF_ILGYNOTH, me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN);
        me->SummonCreature(NPC_DOMINATOR_TENTACLE, me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN);
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
    {
        if (summon->GetEntry() == NPC_EYE_OF_ILGYNOTH)
        {
            ...
            specialPhase = true;
            instance->SetData(DATA_EYE_ILGYNOTH, DONE);
            me->RemoveDynObject(SPELL_DARKEST_NIGHTMARE_AT);
    ...
    void SummomRandPos(uint8 count, uint32 entry)
    {
        for (uint8 i = 0; i < count; ++i)
        {
            ...
            float angle = frand(0.f, 3.14f * 2.f);
    ...
    {
        instance->DoOnPlayers([this, text](Player* player)
        {
            Talk(text, player);
        });
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_DARK_RECONSTITUTION:
                me->CastSpell(me, SPELL_DARKEST_NIGHTMARE_AT, true);
            ...
            case EVENT_2:
                if (Unit* summoner = me->GetOwner())
                    if (Creature* horror = summoner->SummonCreature(NPC_NIGHTMARE_HORROR, sumPos[3]))
                    {
                        ...
                        horror->SetUnitFlags(UNIT_FLAG_NOT_SELECTABLE);
                        horror->SetUnitFlags(UNIT_FLAG_IMMUNE_TO_PC);
                        horror->SetUnitFlags(UNIT_FLAG_NOT_ATTACKABLE_1);
                    }
                Talk(0);
