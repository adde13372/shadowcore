    ...
    struct boss_guldanAI : BossAI
    {
        ...
        void SpellFinishCast(const SpellInfo* spell)
        {
            ...
            case SPELL_FEL_OF_SARGERAS:
            {
                ...
                if (IsNormalRaid() || IsLFR())
                    Trinity::Containers::RandomResize(threatList, (felOfSargerasCasts % 2) + 1); // 2-3
                else
                    Trinity::Containers::RandomResize(threatList, (felOfSargerasCasts % 3) + 1); // 2-4
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SOUL_SIPHON:
                {
                    ...
                    Trinity::Containers::RandomResize(threatList, urand(3, 4));
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_5:
                {
                    ...
                    if (threatList.size() >= 3)
                        Trinity::Containers::RandomResize(threatList, 3);
                ...
                case EVENT_6:
                {
                    ...
                    if (threatList.size() >= 2)
                        Trinity::Containers::RandomResize(threatList, 2);
...
public:
    ...
    struct npc_guldan_eyeAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            me->SetPower(POWER_ENERGY, 0);
            if (me->GetOwner() && me->GetOwner()->IsCreature())
                if (Unit* target = me->GetOwner()->ToCreature()->AI()->SelectTarget(SELECT_TARGET_FARTHEST, urand(0, 2), 0.0f, true))
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_1:
                {
                    ...
                    else if (me->GetOwner() && me->GetOwner()->ToCreature())
                    {
                        ...
                        if (!target)
                            target = me->GetOwner()->ToCreature()->AI()->SelectTarget(SELECT_TARGET_FARTHEST, 0, 0.0f, true);
                        if (target)
                        {
                            targetGUID = target->GetGUID();
