...
struct boss_fallen_avatar : BossAI
{
        ...
        if (me->IsAlive())
            if (Creature* maiden = me->SummonCreature(NPC_AVATARA_MAIDEN, 6583.50f, -771.65f, 1663.67f, 3.04f, TEMPSUMMON_MANUAL_DESPAWN))
                for (const auto pos : pilonesPos)
                    me->SummonCreature(NPC_PILONES, pos, TEMPSUMMON_MANUAL_DESPAWN, 0);
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        if (who)
            AddDelayedCombat(500, [&]() -> void
        {
            if (Creature* maiden = me->FindNearestCreature(NPC_AVATARA_MAIDEN, 200.0f, true))
            {
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        //e->CreateConversation(4955);// 14^09
        AddDelayedEvent(30000, [this]() -> void
        {
            if (Creature* kiljedan = instance->instance->GetCreature(instance->GetGuidData(NPC_KILDJEDAN_AVATARA)))
            {
                ...
                AddDelayedEvent(4000, [kiljedan]() -> void
                {
        ...
        AddDelayedEvent(58000, [this]() -> void
        {
            if (Creature* velen = me->FindNearestCreature(121170, 200.0f, true))
            {
                velen->SetCanFly(false);
                velen->GetMotionMaster()->MovePoint(0, 6748.58f, -795.00f, 1527.00f);
                AddDelayedEvent(4000, [velen]() -> void
                {
        ...
        AddDelayedEvent(115000, [this]() -> void
        {
            for (auto id : { 121171, 121169 })
                if (Creature* add = me->FindNearestCreature(id, 200.0f, true))
                {
                    add->SetCanFly(false);
                    add->GetMotionMaster()->MovePoint(0, 6748.58f, -795.00f, 1527.00f);
                    AddDelayedEvent(3000, [add]() -> void
                    {
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (me->GetDistance(who) < 100.0f && !IntroDone)
        {
            ...
            if (Creature* kiljedan = instance->instance->GetCreature(instance->GetGuidData(NPC_KILDJEDAN_AVATARA)))
            {
                ...
                AddDelayedEvent(27000, [kiljedan, this]() -> void
                {
                    ...
                    AddDelayedEvent(13000, [kiljedan]() -> void
                    {
    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        ...
        {
        case SPELL_CONSUME:
            if (Creature* maiden = me->FindNearestCreature(NPC_AVATARA_MAIDEN, 100.0f, true))
            {
        ...
        case SPELL_ANNIHILATION:
            ...
            AddDelayedCombat(500, [this]() -> void
            {
                ...
                AddDelayedCombat(5000, [this]() -> void
                {
                    ...
                    for (auto pair : perephaseEvent)
                        ...
                        {
                            cre->SetWalk(false);
                           // cre->GetMotionMaster()->MoveSmoothPath(0, pair.second.second);
                            cre->SetCanFly(true);
                        }
                    me->CastSpell(me, SPELL_CONVERS_EVENT_1, false);
                });

                AddDelayedCombat(7000, [this]() -> void // wtf is this shit? there is buildin commands already so why not use them?
                {
                    instance->DoOnPlayers([](Player* player)
                    {
            ...
            for (auto pair : localFloorBlocks)
                if (me->GetDistance(pair.second) <= 30.0f)
                {
                    if (GameObject* go = instance->instance->GetGameObject(instance->GetGuidData(pair.first)))
                        AddDelayedEvent(me->GetDistance(pair.second) * 3 + 1, [go]() -> void
                    {
                        go->SetDestructibleState(GO_DESTRUCTIBLE_DESTROYED, nullptr, false);
                    });
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SHADOWY_BLADES:
            {
                ...
                for (uint8 i = 0; i < (IsMythicRaid() ? 5 : 3); ++i)
                {
                    ...
                    if (Unit* target = (*itr)->getTarget())
                        me->SummonCreature(NPC_CORRUPTED_BLADE, bladesPositions[i], TEMPSUMMON_TIMED_DESPAWN, 11000);
            ...
            case EVENT_UNBOUND_CHAOS:
                ...
                break;
            case EVENT_DESOLATE:
                me->CastSpell(me->GetVictim(), SPELL_DESOLATE, false);
                events.RescheduleEvent(EVENT_DESOLATE, 13000);
                break;
            ...
            case EVENT_CHECK_ENERGY:
                ...
                if ((me->GetPower(me->GetPowerType()) >= 100 && me->FindNearestCreature(NPC_AVATARA_MAIDEN, 100.0f, true) && me->FindNearestCreature(NPC_AVATARA_MAIDEN, 100.0f))
                    || (me->FindNearestCreature(NPC_AVATARA_MAIDEN, 100.0f, true) && !me->FindNearestCreature(NPC_AVATARA_MAIDEN, 100.0f, true)))
                {
                    isSecondPhase = true;
                    events.Reset();
                    DoCast(SPELL_REMOVE_BEAMS);
            ...
            case EVENT_TOUCH_OF_SARGERAS:
            {
                ...
                AddDelayedCombat(2500, [this]() -> void
                {
                    std::vector<Position> positions = sargerasPositions;

                    Trinity::Containers::RandomResize(positions, IsMythicRaid() ? 4 : 3);
            ...
            case EVENT_RAIN:
            {
                ...
                {
                    AddDelayedCombat(i++ * 50 + 10, [this, pair]() -> void
                    {
...
class spell_tos_shadow_blades : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_tos_shadow_blades::HandleDummy, SpellEffIndex::EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_tos_black_winds : public AuraScript
{
    ...
    void OnTick(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Creature* add = caster->SummonCreature(NPC_BLACK_WINDS, start, TEMPSUMMON_TIMED_DESPAWN, 10000))
        {
            ...
            AddDelayedEvent(1000, [add, end]() -> void
            {
