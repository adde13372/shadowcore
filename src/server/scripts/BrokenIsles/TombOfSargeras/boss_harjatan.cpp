...
public:
    ...
    struct boss_harjatanAI : BossAI
    {
        ...
        void Reset() override
        {
            ...
            me->GetAreaTriggerListWithSpellIDInRange(list, 13251, 80.0f);
            ...
            if (IsMythicRaid())
            {
                for (uint8 j = 0; j < 4; ++j)
                {
                    ...
                    for (uint8 i = 0; i < 5; ++i)
                        ...
                        {
                            add->SetUnitFlags(UNIT_FLAG_NOT_SELECTABLE);
                            add->SetUnitFlags(UNIT_FLAG_IMMUNE_TO_PC);
                            add->SetUnitFlags(UNIT_FLAG_IMMUNE_TO_NPC);
                            add->SetUnitFlags(UNIT_FLAG_NOT_ATTACKABLE_1);
                            ...
                            for (const auto& guid : eggs[i])
                                if (Creature* egg = instance->instance->GetCreature(guid))
                                {
                                   ...
                                   // egg->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_NOT_ATTACKABLE_1);
                                    ...
                                    egg->CastSpell(egg, eggsCasts[color], false);
                                }
                        }
                        else
...
class spell_tos_fixate : public AuraScript
{
    ...
    void Register()
    {
        OnEffectApply += AuraEffectApplyFn(spell_tos_fixate::OnApply, EFFECT_0, SPELL_AURA_MOD_FIXATE, AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(spell_tos_fixate::OnRemove, EFFECT_0, SPELL_AURA_MOD_FIXATE, AURA_EFFECT_HANDLE_REAL);
...
class spell_tos_draw_in : public AuraScript
{
    ...
    void OnTick(AuraEffect const* /*aurEff*/)
    {
        ...
        std::list<AreaTrigger*> list;
        caster->GetAreaTriggerListWithSpellIDInRange(list, 13251, 80.0f);
        uint8 realCount = 0;
        for (std::list<AreaTrigger*>::const_iterator itr = list.begin(); itr != list.end(); itr++)
        {
        ...
        for (uint8 i = 0; i < realCount; ++i)
            caster->CastSpell(caster, SPELL_FRIGID_BLOWS, true);
    }
