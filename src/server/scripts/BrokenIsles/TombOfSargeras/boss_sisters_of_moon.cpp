...
struct boss_sisters_of_the_moon : BossAI
{
    explicit boss_sisters_of_the_moon(Creature* creature) : BossAI(creature, DATA_SISTERS_OF_THEMOON)
    {
        if (me->IsAlive())
        {
            ...
            AddDelayedCombat(500, [=]() -> void { DoAction(ACTION_1); });
        }
    }
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type == EFFECT_MOTION_TYPE)
        {
            ...
            case SPELL_SABERLEAP:
                AddDelayedCombat(500, [=]() -> void { me->SetFacingTo(centrPos.GetAngle(me) - M_PI); });
                break;
            }
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_MOON_GLAIVE:
                me->CastSpell(me->GetVictim(), SPELL_MOON_GLAIVE);
...
struct npc_sister_lunaspyre : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        case 1:
            ...
            break;
        case 3:
            //->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
    ...
    void SpellFinishCast(const SpellInfo* spell) 
    {
        ...
        case SPELL_INCORPOREAL_TELEPORT:
            AddDelayedCombat(1000, [=]() -> void { me->SetFacingTo(centrPos.GetAngle(me) - M_PI); });
            break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_LUNAR_FIRE:
                me->CastSpell(me->GetVictim(), SPELL_LUNAR_FIRE);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_TWILIGHT_VOLLEY:
                if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, [this](Unit const* t) { return t->IsPlayer(); }))
                {
                    me->SetFacingToObject(target);
...
class spell_sistersmoon_incorporeal_shot_filter : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_sistersmoon_incorporeal_shot_filter::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_THREAT_LIST);
...
class spell_sistersmoon_embrace_eclipse : public AuraScript
{
    ...
    void Register() override
    {
        ...
        OnEffectAbsorb += AuraEffectAbsorbFn(spell_sistersmoon_embrace_eclipse::Absorb, EFFECT_0);
        OnEffectAbsorb += AuraEffectAbsorbFn(spell_sistersmoon_embrace_eclipse::Absorb, EFFECT_0);
    }
};
