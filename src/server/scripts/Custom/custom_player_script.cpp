...
enum GenericMissingPlayerSpells
{
    ...
    SPELL_GEN_HUNTER_REVIVE_PET = 982,
...
/* ShadowCore Free Repack Welcome message */
/*class ShadowCoreFreeRepackWelcome : public PlayerScript
{
public:
    ShadowCoreFreeRepackWelcome() : PlayerScript("ShadowCoreFreeRepackWelcome") { }

    void OnLogin(Player* player, bool) override { WelcomeMessage(player); }
    void OnQuestAccept(Player* player, Quest const*) override { WelcomeMessage(player); }
    void OnLevelChanged(Player* player, uint8) override { WelcomeMessage(player); }
    void OnMapChanged(Player* player) override { WelcomeMessage(player); }

    void WelcomeMessage(Player* player)
    {
        ChatHandler(player->GetSession()).SendSysMessage("|cffFF0000 This is ShadowCore Free Test Repack. Donate to remove ads and limitations|r");
        ChatHandler(player->GetSession()).SendSysMessage("|cffFF0000 ShadowCore official discord: https://discord.gg/57D59ed ShadowCoreTeam@yahoo.com|r");
        player->GetSession()->SendNotification("This is ShadowCore Free Test Repack. Donate to remove ads and limitations. ShadowCore official discord: https://discord.gg/57D59ed ShadowCoreTeam@yahoo.com");
    }
};*/
...
void AddSC_custom_player_script()
{
    ...
    //RegisterPlayerScript(ShadowCoreFreeRepackWelcome);
}
