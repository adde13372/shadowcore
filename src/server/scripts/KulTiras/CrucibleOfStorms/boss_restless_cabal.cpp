...
public:
	...
	struct bfa_boss_zaxasj_the_speaker_AI : public BossAI
	{
		...
		{
			// //if (CAST_AI(bfa_boss_fathuul_the_feared::bfa_boss_fathuul_the_feared_AI, fathuul->AI())->isDead == true) TEMPFIX
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_OCEANIC_ESSENCE:
				{
					...
					me->CastSpell(spawnPos, SPELL_OCEANIC_ESSENCE_MISSILE);
