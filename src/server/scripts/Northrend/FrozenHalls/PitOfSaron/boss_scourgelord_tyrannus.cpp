    ...
    struct boss_tyrannusAI : public BossAI
    {
        ...
        void KilledUnit(Unit* victim) override
        {
            if (victim->GetTypeId() == TYPEID_PLAYER)
                me->Yell(RAND(SAY_SLAY_1, SAY_SLAY_2), me);
        }

        void JustDied(Unit* /*killer*/) override
        {
            _JustDied();
            me->Yell(SAY_DEATH, me);
        ...
        void UpdateAI(uint32 diff) override
        {
            if (m_startPhaseIntroTyr)
            {
                if (m_uiAtackTimer < diff)
                {
                    ...
                    {
                    case 0:
                        me->Yell(SAY_TYRANNUS_INTRO_1, me);
                    ...
                    case 2:
                        ...
                        {
                            me->Yell(SAY_GORKUN_INTRO_2, pMartin);
                        }
                        if (Creature* pGorkun = GetClosestCreatureWithEntry(me, 37581, 1525.0f))
                        {
                            me->Yell(SAY_GORKUN_INTRO_2, pGorkun);
                        }
                        ++m_uiAtackPhase;
                        m_uiAtackTimer = 8000;
                        break;
                    case 3:
                        me->Yell(SAY_TYRANNUS_INTRO_3, me);
                        ++m_uiAtackPhase;
                        m_uiAtackTimer = 7000;
                        break;
                    case 4:
                        if (Creature* rimeflang = GetClosestCreatureWithEntry(me, 36661, 325.0f))
                        {
                            ...
                            rimeflang->SetUnitFlags(UNIT_FLAG_NOT_SELECTABLE);
                            rimeflang->SetUnitFlags(UNIT_FLAG_NON_ATTACKABLE);
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        me->Yell(SAY_DARK_MIGHT_1, me);
                        me->Yell(SAY_DARK_MIGHT_2, me);
                        DoCast(me, SPELL_UNHOLY_POWER);
                        m_uiAtackPhase = urand(5, 6);
                        m_uiAtackTimer = 20000;
                        break;
                    case 8:
                        me->Yell(SAY_MARK_RIMEFANG_1, me);
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                        {
                            me->Yell(SAY_MARK_RIMEFANG_2);
...
public:
    ...
    {
        boss_rimefangAI(Creature* creature) : BossAI(creature, DATA_RIMEFANG)
        {
            m_bIsRegularMode = creature->GetMap()->IsNormal();
        }
...
public:
    ...
    class spell_tyrannus_overlord_brand_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            //GetTarget()->SetAI(player_overlord_brandAI(GetTarget()->ToPlayer()));
        ...
        void UpdateAI(uint32 diff) override
        {
            if (m_uiSpeech_Timerz)
            {
                if (m_uiSpeech_Timerz <= diff)
                {
                    ...
                    case 1:
                        TeleportPlayers();
                        if (Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, 37581, 200.0f))
                        {
                            pDefenderNPC->CastSpell(pDefenderNPC, SPELL_FROST_BOMB, false);
                        }
            ...
            if (instance->GetBossState(DATA_TYRANNUS) == DONE)
            {
                if (m_uiSpeech_Timer < diff)
                {
                    ...
                    case 1:
                        ...
                        {
                            me->Yell(SAY_OUTRO3_HORDE, me);
                        }
                        if (creatureEntry == NPC_JAINA_PART2)
                        {
                            me->Yell(SAY_OUTRO3_ALY, me);
                        }
                        ++m_uiOutro_Phase;
            ...
            if (instance->GetBossState(DATA_ICK) == DONE && instance->GetBossState(DATA_GARFROST) == DONE && instance->GetBossState(DATA_TYRANNUS) == NOT_STARTED)
            {
                if (m_uiGauntletTimer < diff)
                {
                    ...
                    case 0:
                        switch (urand(0, 1))
                        {
                        case 0: me->Yell(SAY_GAUNTLET1, me); break;
                        case 1: me->Yell(SAY_GAUNTLET2, me); break;
                        }
