...
public:
    ...
    class spell_dru_wild_growth_SpellScript : public SpellScript
    {
        ...
        bool Load() override
        {
            maxTargets = 6; // based on value on EFFECT_2
            return true;
        }
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                // Sort list on health order (ascending), if more than 6 (or equal) resize to 6, fewer than 6,just resize the list on healpct order (can be 1-5) 
        ...
        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            if (caster != target)
                if (caster->CastSpell(target, SPELL_DRUID_MOONFIRE_DAMAGE, true))
                    AddPct(damage, sSpellMgr->GetSpellInfo(SPELL_DRUID_MOONFIRE_DAMAGE, DIFFICULTY_NONE)->GetEffect(EFFECT_0)->BasePoints);
...
public:
    ...
    class spell_dru_travel_form_dummy_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckCast()
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(player->IsInWater() ? DRUID_AQUATIC_FORM : DRUID_STAG_FORM, DIFFICULTY_NONE);
...
public:
    ...
    class spell_dru_moonfire_sunfire_dot_AuraScript : public AuraScript
    {
        ...
        void OnTick(AuraEffect const* /*aurEff*/)
        {
            SpellInfo const* shootingStars = sSpellMgr->AssertSpellInfo(SPELL_DRU_SHOOTING_STARS, DIFFICULTY_NONE);
...
public:
    ...
    class spell_dru_t10_balance_4p_bonus_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_DRUID_LANGUISH, DIFFICULTY_NONE);
