...
class spell_gen_break_shield : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_break_shield_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                OnEffectHitTarget += SpellEffectFn(spell_gen_break_shield_SpellScript::HandleScriptEffect, uint8(EFFECT_FIRST_FOUND), SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };
...
class spell_gen_clone : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_clone_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                // Have them one by one, if one has their hook changed, it will ruin all of them, we don't want that...
...
class spell_gen_despawn_self : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_despawn_self_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                OnEffectHitTarget += SpellEffectFn(spell_gen_despawn_self_SpellScript::HandleDummy, uint8(EFFECT_ALL), SPELL_EFFECT_ANY);
            }
        };
        ...
        class spell_gen_mounted_charge_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                ...
                if (spell->HasEffect(SPELL_EFFECT_SCRIPT_EFFECT))
                    OnEffectHitTarget += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleScriptEffect, uint8(EFFECT_FIRST_FOUND), SPELL_EFFECT_SCRIPT_EFFECT);
...
class spell_gen_replenishment : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_replenishment_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_replenishment_SpellScript::RemoveInvalidTargets, uint8(EFFECT_ALL), TARGET_UNIT_CASTER_AREA_RAID);
            }
        };
...
public:
    ...
    private:
        ...
        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_gen_mixology_bonus_AuraScript::CalculateAmount, uint8(EFFECT_ALL), SPELL_AURA_ANY);
        }
    };
...
class spell_gen_eredar_bloodmage_blood_siphon_damage : public SpellScript
{
    ...
    void CalculateDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        float healthPctDmg = target->CountPctFromCurHealth(GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints);
...
public:
    ...
    void OnDeath(Player* player) override
    {
        if (player->HasAura(SPELL_LIGHT_RECKONING))
            if (SpellInfo const* info = sSpellMgr->GetSpellInfo(SPELL_LIGHT_RECKONING, DIFFICULTY_NONE))
...
public:
    ...
    class spell_gen_dummy_trigger_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* /*SpellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_PERSISTANT_SHIELD_TRIGGERED, DIFFICULTY_NONE) || !sSpellMgr->GetSpellInfo(SPELL_PERSISTANT_SHIELD, DIFFICULTY_NONE))
                return false;
            return true;
        }
...
public:
    ...
    public:
        ...
        bool Validate(SpellInfo const* /*spell*/) override
        {
            if (_mount0 && !sSpellMgr->GetSpellInfo(_mount0, DIFFICULTY_NONE))
                return false;
            if (_mount60 && !sSpellMgr->GetSpellInfo(_mount60, DIFFICULTY_NONE))
                return false;
            if (_mount100 && !sSpellMgr->GetSpellInfo(_mount100, DIFFICULTY_NONE))
                return false;
            if (_mount150 && !sSpellMgr->GetSpellInfo(_mount150, DIFFICULTY_NONE))
                return false;
            if (_mount280 && !sSpellMgr->GetSpellInfo(_mount280, DIFFICULTY_NONE))
                return false;
            if (_mount310 && !sSpellMgr->GetSpellInfo(_mount310, DIFFICULTY_NONE))
                return false;
            return true;
        }
...
public:
    ...
    class spell_brewfest_speed_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_brewfest_speed_AuraScript);
            bool Validate(SpellInfo const * /*SpellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_RAM_FATIGUE, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_BREWFEST_RAM, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SPEED_RAM_GALLOP, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SPEED_RAM_CANTER, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SPEED_RAM_TROT, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SPEED_RAM_NORMAL, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SPEED_RAM_GALLOP, DIFFICULTY_NONE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_SPEED_RAM_EXHAUSED, DIFFICULTY_NONE))
                return false;
            return true;
        }
...
public:
    ...
    class spell_gen_battle_guild_standart_AuraScript : public AuraScript
    {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_gen_battle_guild_standart_AuraScript::HandleTriggerSpell, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
    ...
    class spell_gen_herbalism_trap_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_gen_herbalism_trap_SpellScript::HandleDummy, uint8(EFFECT_ALL), SPELL_EFFECT_DUMMY);
        }
    };
...
public:
    ...
    class spell_gen_mining_trap_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_gen_mining_trap_SpellScript::HandleDummy, uint8(EFFECT_ALL), SPELL_EFFECT_DUMMY);
        }
    };
