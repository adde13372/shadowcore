...
public:
    ...
    class spell_hun_masters_call_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* spellInfo) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_HUNTER_MASTERS_CALL_TRIGGERED, DIFFICULTY_NONE) ||
                !sSpellMgr->GetSpellInfo(spellInfo->GetEffect(EFFECT_0)->CalcValue(), DIFFICULTY_NONE))
                return false;
            return true;
        }
    ...
    class spell_hun_beast_cleave_proc_AuraScript : public AuraScript
    {
        ...
        void OnProc(const AuraEffect* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                ...
                {
                    float bp = eventInfo.GetDamageInfo()->GetDamage() * 0.75f;
    ...
    class spell_hun_marked_shot_SpellScript : public SpellScript
    {
        ...
        void HandleOnCast()
        {
            if (Unit* caster = GetCaster())
            {
                ...
                float radius = sSpellMgr->GetSpellInfo(SPELL_HUNTER_MARKED_SHOT, DIFFICULTY_NONE)->GetEffect(0)->CalcRadius(caster);
...
public:
    ...
    struct at_hun_sentinelAI : AreaTriggerAI
    {
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            {
                std::list<Unit*> targetList;
                float radius = sSpellMgr->GetSpellInfo(SPELL_HUNTER_SENTINEL, DIFFICULTY_NONE)->GetEffect(0)->CalcRadius(caster);
...
public:
    ...
    class spell_hun_pet_heart_of_the_phoenix_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_HUNTER_PET_HEART_OF_THE_PHOENIX_TRIGGERED, DIFFICULTY_NONE) || !sSpellMgr->GetSpellInfo(SPELL_HUNTER_PET_HEART_OF_THE_PHOENIX_DEBUFF, DIFFICULTY_NONE))
                return false;
            return true;
        }
                ...
                if (Unit* mainTarget = GetHitUnit())
                {
                    ...
                    if (!targetList.empty())
                    {
                        ...
                        for (auto victim : victimList)
                        {
                            ...
                            mainTarget->CastCustomSpell(victim, 164340, &damagePct, nullptr, true, nullptr, nullptr, modOwner->GetGUID());
                        }
                    }
                }
            }
