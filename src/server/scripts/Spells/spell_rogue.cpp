...
class aura_rog_venomous_wounds : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& /*procInfo*/)
    {
        ...
        if (caster->HasAura(SPELL_ROGUE_VENOM_RUSH))
            energyGain += sSpellMgr->GetSpellInfo(SPELL_ROGUE_VENOM_RUSH, DIFFICULTY_NONE)->GetEffect(EFFECT_0)->BasePoints;
        ...
        if(Unit* caster = GetCaster())
        {
            if(Unit* target = GetHitUnit())
            {
                ...
                if (roll_chance_i(chance))
                {
                    ...
                    SpellNonMeleeDamage dmg(caster, target, GetSpellInfo(), { GetSpellInfo()->GetSpellXSpellVisualId(), 0 }, GetSpellInfo()->SchoolMask);
